...
@php
    ...
    $imageUrl = is_string($url) && trim($url) !== '' ? trim($url) : null;

    if ($imageUrl !== null) {
        $containerStyle = sprintf(
            'width:%dpx;height:%dpx;border-radius:%dpx;background:var(--w-cream-2);position:relative;overflow:hidden;flex-shrink:0;box-shadow:inset 0 0 0 0.5px var(--w-line);',
            $sizePx,
            $sizePx,
            $radiusPx,
        );
...
<div {{ $attributes->merge(['style' => $containerStyle, 'role' => 'img', 'aria-label' => $brand ? $brand.' product image' : 'Product image']) }}>
    @if ($imageUrl)
        <img
            src="{{ $imageUrl }}"
            alt=""
            ...
            data-testid="product-image-photo"
